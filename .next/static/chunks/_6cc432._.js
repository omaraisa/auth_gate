...
function LoginForm() {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "LoginForm.useEffect": ()=>{
            const cookie = document.cookie.split(';').map({
                "LoginForm.useEffect.cookie": (c)=>c.trim()
            }["LoginForm.useEffect.cookie"]).find({
                "LoginForm.useEffect.cookie": (c)=>c.startsWith('arcgis_token')
            }["LoginForm.useEffect.cookie"]);
            console.log('Extracted :', document.cookie);
            if (cookie) {
                const token = cookie.split('=')[1];
                console.log('Extracted token:', token);
            // IdentityManager.registerToken({ ... })
            } else {
                console.log('No arcgis_token cookie found.');
            }
        }
    }["LoginForm.useEffect"], []);
